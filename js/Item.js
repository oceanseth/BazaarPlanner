...
/*
            ...
            conditionalMatches.forEach(conditionalMatch=>{
                switch(conditionalMatch.toLowerCase()) {
                    ...
                    case "an adjacent item slows or freezes":
                        const adjItems = this.getAdjacentItems();
    ...
    */
    ...
    getAnonymousTriggerFunctionFromText(text) {        
        ...
        if(match) {
            const gainAmount = getRarityValue(match[1], this.rarity);
            return (item)=>{
                (item||this).getAdjacentItems().forEach(item => {
                    if(item.tags.includes("Weapon")) {  
   ...
   //     this.getAmmoTriggerFunctionFromText(text) ||
    ...
    async generateEnchantOverlay() {
        // Cache check
        if (!Item.generatedImages) Item.generatedImages = {};
        if (!Item.generatedImages[this.name]) Item.generatedImages[this.name] = {};
        if (Item.generatedImages[this.name][this.enchant]) return Item.generatedImages[this.name][this.enchant];
    ...
    async resetEnchant() {
        if (this.enchant) {
            ...
            if (Item.enchantTagMap[this.enchant] && !this.tags.includes(Item.enchantTagMap[this.enchant])) {
                this.tags.push(Item.enchantTagMap[this.enchant]);
            }
            this.tags.push("Enchanted");
            this.element.classList.add(this.enchant);

            // Generate and apply enchant overlay
            const overlay = await this.generateEnchantOverlay();
            if (overlay) {
                this.element.style.setProperty('--enchant-overlay', `url(${overlay})`);
    ...
    static async detectEdges(imageData) {
        const edges = [];
